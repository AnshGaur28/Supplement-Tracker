...
export default function App() {
  ...
  return (
    <Box p={2} maxWidth={600} mx="auto" textAlign="center" mb={4}>
      <Typography
        variant="h3"
        component="h1"
        gutterBottom
      ...
      {/* User Select */}
      <Select value={user} onChange={(e) => setUser(e.target.value)} sx={{ mt: 2 }}>
        {USERS.map((u) => (
      ...
      <Select value={filter} onChange={(e) => setFilter(e.target.value)} sx={{ ml: 2 }}>
        ...
        {Array.from(
          new Set([...SUPPLEMENTS_PLAN.weekdays, ...SUPPLEMENTS_PLAN.dailyDinesh])
        ).map((supp) => (
        ...
        <Box>
          ...
          <Box mt={3}>
            <Typography variant="h6" gutterBottom>
              Supplements for {format(selectedDate, "MMMM dd, yyyy")}
            </Typography>
            {Array.from(
              new Set([...SUPPLEMENTS_PLAN.weekdays, ...SUPPLEMENTS_PLAN.dailyDinesh])
            ).map((supp) => {
              const plannedForToday = todaySupplements.includes(supp);
              const checked = data[format(selectedDate, "yyyy-MM-dd")]?.includes(supp) || false;
                ...
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      checked={checked}
                      onChange={() =>
                        plannedForToday && toggleSupplementForDate(selectedDate, supp)
                      }
            ...
            <Box display="flex" justifyContent="space-between" mb={2}>
              <Button
                ...
                onClick={() => {
                  ...
                  setCurrentMonth(prev >= minMonth ? prev : currentMonth);
                }}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setCurrentMonth(next <= maxMonth ? next : currentMonth);
                }}
            ...
            <CalendarHeatmap
              ...
              values={Array.from(
                ...
                (_, i) => {
                  ...
                  const takenPlannedSupps = supplements.filter((supp) =>
                    planned.includes(supp)
                  );

                  const relevantSupps =
                    filter === "All"
                      ? takenPlannedSupps
                      : takenPlannedSupps.includes(filter)
                      ? [filter]
                      : [];
              ...
              classForValue={(value) => {
                ...
                if (
                  selectedDate &&
                  isSameDay(valueDate, new Date(selectedDate))
                )
          ...
          {/* Edit Modal */}
          <Dialog open={!!editDate} onClose={() => setEditDate(null)} maxWidth="xs" fullWidth>
            <DialogTitle>Edit Supplements – {editDate}</DialogTitle>
            <DialogContent>
              {Array.from(
                new Set([...SUPPLEMENTS_PLAN.weekdays, ...SUPPLEMENTS_PLAN.dailyDinesh])
              ).map((supp) => {
                const plannedForDate = getPlannedSupplements(editDate, user).includes(supp);
                return (
                  <FormControlLabel
                    key={supp}
                    control={
              ...
              <Box mt={2} display="flex" justifyContent="flex-end" gap={1}>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const newData = { ...data, [editDate]: updatedDayData };
                    setData(newData);
                    localStorage.setItem(`supp-${user}`, JSON.stringify(newData));
                    fetch("/api/set", {
                      method: "POST",
